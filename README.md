...
- **带文字预览**：预览图先允许 image2 生成带文字的页面，方便用户判断版式、文字密度和整体效果。
...
- **透明文本框**：后续添加的 PPT 文本框默认无填充色、无不透明背景、无不必要边框。
- **文本自适应背景**：字体、字号、行距、对齐方式和位置应尽量与 image2 生成背景协调。
- **必要时重写文本**：如果原文过长或放入页面不协调，可以基于源文档改写成更适合幻灯片的短文本，但不能改变事实、数字、名称、日期或核心结论。
...
5. **生成带文字的 image2 例图**  
   先生成 2-3 张代表性完整 PPT 页面例图。例图中可以包含标题、标签和简短正文，让用户检查文字位置、层级、密度和整体构图。

6. **制作参考 PPT 页**  
   用户确认例图后，把例图转换成参考 PPT 页。此时要以 image2 例图为视觉参考，把图片里的文字重建为透明、可编辑的 PPT 文本框，并检查图表、表格、数据是否可编辑。

7. **批量生成完整页面**  
   用户确认参考 PPT 页后，再批量生成每一页的完整 PPT 页面 PNG。正式页面也可以包含可见文字，用来确定最终版式和视觉层级。
...
10. **检查和修改**  
   交付前检查内容是否忠于文档、画风是否统一、关键元素是否可编辑、文本框是否透明且与背景协调。后续修改时优先按用户最新要求，只改指定页面。
...
先给我页数建议和每页大纲；我确认后再生成带文字的 image2 例图。
例图确认后，先做参考 PPT 页让我检查可编辑性。
文本、图表、表格和数据必须可编辑；后加文本框要透明并适配背景。
复杂背景可以整体保留。
严格依据文档，不要编造内容。
